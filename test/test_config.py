...
def test_invalid_default_encoding():
    with pytest.raises(ConfigurationError, match="default_encoding is not a valid encoding"):
        config = CalculatorConfig(default_encoding="not-a-real-encoding")
        config.validate()
...
def test_validate_creates_directories():
